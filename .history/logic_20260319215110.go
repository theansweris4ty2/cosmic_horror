...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	srcY := g.player.animationFrame / 4

	opts.GeoM.Translate(g.player.x, g.player.y)
	screen.DrawImage(g.player.playerImg.SubImage(image.Rect(srcX, srcY, 16, 16)).(*ebiten.Image), opts)
	opts.GeoM.Reset()
