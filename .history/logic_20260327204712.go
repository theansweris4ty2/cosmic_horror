...
func (g *Game) Draw(screen *ebiten.Image) {
	offScreenImage := ebiten.NewImage(900, 630)
	...
	for i := -1; i < 3; i++ {
		...
		// offScreenImage.DrawImage(g.background, opts)
		// opts.GeoM.Reset()
		// opts.GeoM.Translate(g.camera.x, g.camera.y)
		opts.GeoM.Translate(g.player.x, g.player.y)
		g.background.DrawImage(g.player.playerImages[g.player.action].SubImage(image.Rect(srcX, srcY, srcX+128, srcY+128)).(*ebiten.Image), opts)
		opts.GeoM.Reset()
		offScreenImage.DrawImage(g.background, opts)
		opts.GeoM.Reset()
		opts.GeoM.Translate(g.camera.x, g.camera.y)
	...
	// opts.GeoM.Translate(g.camera.x, g.camera.y)
	// opts.GeoM.Translate(g.player.x, g.player.y)
	// offScreenImage.DrawImage(g.player.playerImages[g.player.action].SubImage(image.Rect(srcX, srcY, srcX+128, srcY+128)).(*ebiten.Image), opts)
	// opts.GeoM.Reset()

	// opts.GeoM.Translate(g.monster.x, g.monster.y)
	// opts.GeoM.Translate(g.camera.x, g.camera.y)
	// offScreenImage.DrawImage(g.monster.monsterImages[0].SubImage(image.Rect(msrcX, msrcY, msrcX+128, msrcY+128)).(*ebiten.Image), opts)
	// opts.GeoM.Reset()
	screen.DrawImage(offScreenImage, opts)
}
...
func (p *Player) movement(g *Game) {
	...
	g.monster.veloX = -1
	if g.timer%10 == 0 && g.monster.animationFrame > 0 {
