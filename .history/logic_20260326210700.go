...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	for i := 0; i < 10; i++ {
		opts.GeoM.Translate((g.camera.x+50)+float64(i*384), g.camera.y)
		screen.DrawImage(g.background, opts)
		opts.GeoM.Reset()
	}

	opts2 := &ebiten.DrawImageOptions{}
	opts2.GeoM.Translate(g.player.x, g.player.y)
	screen.DrawImage(g.player.playerImages[g.player.action].SubImage(image.Rect(srcX, srcY, srcX+128, srcY+128)).(*ebiten.Image), opts2)
	opts2.GeoM.Reset()
